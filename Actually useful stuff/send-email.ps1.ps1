...
try {
    ...
    # Rubbish little check to see if we're already connected to MS Graph
    # If we can't search MS Graph then we are going to set up the connection vars and connect.
    # Connection is done with a certificate provided by the CA
    if (!(Get-MgUser -UserId all.itinfrastructure@unitestudentsyammer.onmicrosoft.com -ErrorAction SilentlyContinue)) {
